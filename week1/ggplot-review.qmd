...
ggplot(penguins, 
  ...
  geom_point() +
  scale_color_manual(values = c("lightpink", "darkorange", "darkgreen"))
```
...
ggplot() +
  ...
  geom_point(data = penguins_summary, 
             mapping = aes(mean_bill_length_mm, mean_bill_depth_mm, fill = species), 
             size = 5,
             shape = 24)
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  theme_light() +
  theme(axis.title = element_text(size = 15, color = "navy"))
```
...
(p1 + (p2 / p3)) +
  plot_annotation(title = "All Things Penguins")
```
